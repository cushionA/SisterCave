...
///・連携アクションと仲良くするために頑張らないと。中断と再開の仕様
///・要修正とか未実装で検索したらダメなとこ出てくるよ
///・あとは標的死亡時のキャンセル処理も。エネミーマネージャーから通知飛ばしてそれが今のターゲットと合致するかを確認（今は攻撃モードで、なおかつ番号が一致するとか）
///
///
///
...
namespace MoreMountains.CorgiEngine // you might want to use your own namespace here
{
	...
	public class FireAbility : MyAbillityBase
    {
        ...
        //ビット演算でクールタイムを破棄する
        SisterConditionBase.SkipCondition skipCondition = SisterConditionBase.SkipCondition.なし;
        ...
        public void StateInitialize(bool battle)
        {
            ...
            else
            {
                ...
                //回復に行動変化
                SisterStateChange(SisterParameter.MoveType.回復);
        ...
        async UniTaskVoid UseMagic(TargetData target, SisMagic useMagic)
        {
            ...
            //ここアリ－リストからナンバーで飛ばせる
            if (target.targetObj != null && sb.nowMode == SisterParameter.MoveType.攻撃)
            {
            ...
            if (stateChange && sister.priority != sb.nowMode)
            {
                //クールタイムキャンセル
                disEnable = false;

                //状態変化
                SisterStateChange(sister.priority);
                stateChange = false;
                StateController().Forget();
            }
        ...
        {
            //乱数生成のもとになる数値をきれいに
            UnityEngine.Random.InitState((int)(GManager.instance.nowTime * 1000));
        ...
        public void CastCircleCall(Magic useMagic, float targetPosition)
        {
            ...
            //ターゲットの方向を向く
            sb.SisFlip(Mathf.Sign(targetPosition - SManager.instance.sisterPosition.x));
        ...
        {
            if (sb.nowMode == SisterParameter.MoveType.攻撃)
            {
                return (SManager.instance._targetList[targetNum], SManager.instance.attackMagi[useMagicNum]);
            }

            else if (sb.nowMode == SisterParameter.MoveType.支援)
            {
        ...
        /// </summary>
        void SisterStateChange(SisterParameter.MoveType condition)
        {
            sb.CombatModeChange(condition);
        ...
        /// ターゲットと使用魔法をintで保持する
        /// </summary>
        async UniTaskVoid CombatActionJudge()
        {
            ...
            //もし通常状態じゃないならそれまで魔法開始を待つ
            if (!sb.AttackableCheck())
            {
                await UniTask.WaitUntil(() => sb.AttackableCheck(), cancellationToken: magicCancel.Token);
            }
            ...
            if (useCondition.selectAction == UseAction.なにもしない)
            {
                //クールタイムあるなら待つ
                //クールタイム中は魔力が急速回復する
                if(useCondition.coolTime > 0)
                {
                    CoolTimeWait(useCondition.coolTime).Forget();
                }
        ...
        bool NecessaryCheck(bool mpCheck, bool RangeCheck)
        {
            ...
            if (sb.nowMode == SisterParameter.MoveType.攻撃)
            {
                distance = Vector2.SqrMagnitude(SManager.instance.sisterPosition- SManager.instance._targetList[targetNum]._condition.targetPosition);

            }
            else
            {
                distance = Vector2.SqrMagnitude(SManager.instance.sisterPosition - EnemyManager.instance._targetList[targetNum]._condition.targetPosition);
            }

            //このdistanceは二乗してあるので、射程範囲も二乗してね
            ...
            for (int i = 0; i < magicCount; i++)
            {
                ...
                if (RangeCheck)
                {
                    ...
                    //なんか魔法の情報に射程がわかるような変数積んどこう
                    //if()して偽ならmp調べずにコンティニュー
                }
        ...
        public bool ActConditionJudge(ActJudgeCondition[] judgeData, UseAction action)
        {
            ...
            if (noCondition)
            {
                ...
                if (stateChange)
                {
                    SisterStateChange(GetMoveType(action));
                    return false;
                }
                return true;
            }
            else
            {
                ...
                //条件当てはまるなら状態変化か行動を実行
                if (JudgeStart(judgeData, sb.nowMode == SisterParameter.MoveType.攻撃, action))
                {
        ...
        /// <summary>
        /// 選択した行動から行動タイプを返すメソッド
        /// </summary>
        /// <param name="action"></param>
        /// <returns></returns>
        SisterParameter.MoveType GetMoveType(UseAction action)
        {
            if(action == UseAction.攻撃行動に移行)
            {
                return SisterParameter.MoveType.攻撃;
            }
            else if (action == UseAction.支援行動に移行)
            {
                return SisterParameter.MoveType.支援;
            }
            else
            {
                return SisterParameter.MoveType.回復;
            }
        }
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                else
                {
                    ...
                    for (int s = 0; s < count; s++)
                    {
                        ...
                        {
                            //含んでない要素をremoveしてもエラーじゃないので判断不要
        ...
        bool JudgeExe(TargetData data, ActJudgeCondition condition)
        {
            ...
            if ((int)condition.content < 3)
            {
                ...
                {
                    //タイプは一致してる？
                    //タイプがそのまま残ってるなら条件に自分のタイプが含まれてるってことよ
                    return ((int)data._baseData._type & condition.percentage) == (int)data._baseData._type;
                }
            }
            else if ((int)condition.content < 6)
            {
                ...
                {
                    //バフを含んでいるか
                    return ((int)data._condition.buffImfo & condition.percentage) == (int)data._condition.buffImfo;
                }
                else if (condition.content == CheckContent.negaCondition)
                {
                    //デバフを含んでいるか
                    return((int)data._condition.debuffImfo & condition.percentage) == (int)data._condition.debuffImfo;
                }
            ...
            else if ((int)condition.content < 9)
            {
                ...
                {
                    return ((int)data._baseData.WeakPoint & condition.percentage) == (int)data._baseData.WeakPoint;
                }
                else if (condition.content == CheckContent.distance)
                {

                    //ここキャッシュに置き変えろよほんと
                    float distance = Vector2.SqrMagnitude(data._condition.targetPosition - SManager.instance.sisterPosition);

                    float judgeBase = condition.percentage * condition.percentage;

                    //同じならオッケー
                    if (distance == judgeBase)
                    {
                        return true;
                    }
                    ...
                    //距離が以下ならHighLowが偽なら真になる
                    //以上なら真だと真になる
                    return (distance <= judgeBase) ? !condition.highOrLow : condition.highOrLow;
                }
        ...
        int TargetSelectStart(in TargetSelectCondition condition)
        {
            ...
            //直接指定
            if(condition.SecondCondition == AdditionalJudge.シスターさん)
            {
                return 1;
            }
            else if (condition.SecondCondition == AdditionalJudge.プレイヤー)
            {
                return 0;
            }
            ...
            //攻撃中かどうか
            bool isAttack = sb.nowMode == SisterParameter.MoveType.攻撃;
        ...
        float ReturnValue(AdditionalJudge condition, int num, bool isAttack)
        {
            ...
            else if (condition == AdditionalJudge.ターゲットの距離)
            {
                //ここはキャッシュに置き換える
                //遠近比較だけなら平方根走らせなくていい
                return Vector2.SqrMagnitude(SManager.instance.sisterPosition - data._condition.targetPosition);
            }
        ...
        bool SupportConditionJudge(Magic.SupportType condition)
        {
            ...
            //今支援状態か
            bool isSupport = sb.nowMode == SisterParameter.MoveType.支援;
        ...
        {
            if(condition == AtEffectCon.Element.弱点属性)
            {
                //攻撃対象や味方の弱点属性に置き換える
                if(sb.nowMode == SisterParameter.MoveType.攻撃)
                {
                    condition = SManager.instance._targetList[targetNum]._baseData.WeakPoint;
                }
                else
                {
                    condition = EnemyManager.instance._targetList[targetNum]._baseData.WeakPoint;
                }
            }
            ...
            {
                //弾丸の特徴を照会
                //andしたbitがゼロ以上なら重なってる
                return ((int)SManager.instance.supportMagi[num].magicElement & (int)condition) > 0;
            }
            else
            {
                //弾丸の特徴を照会
                return ((int)SManager.instance.attackMagi[num].magicElement & (int)condition) > 0;
            }
        }
