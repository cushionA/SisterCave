	...
	public class FireAbility : MyAbillityBase
{
		...
		protected const string _attackParameterName = "Attack";
		protected int _attackAnimationParameter;

		//詠唱中かどうか
		protected const string _castParameterName = "Cast";
	...
	protected override void HandleInput()
	{
		...
		if (status.equipCombination != null && !GManager.instance.isDown && !GManager.instance.onGimmick)
		{
			if (status.equipCombination != null && _inputManager.CombinationButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
			{
				...
				if (_controller.State.IsGrounded && combiEnable && !useMagic) //&& !(SManager.instance.actNow && !SManager.instance.castNow))
				{
				...
				else if (!combiEnable && combinationCool >= 1f && status.equipCombination.useMP <= sb.mp)
				{
					//	Debug.Log($"お肉売り場{conboChain}");
					combinationCool = 0;
					conboChain = 0;
					...
					SManager.instance.actNow = true;
					SManager.instance.castNow = false;
					sb.mp -= status.equipCombination.useMP;
					Combination();
					cCounter.text = ((int)status.equipCombination.coolTime).ToString();
					if (cIcon.gameObject.activeSelf)
					{
	...
	void FireAct()
		{
			...
			//コンビネーション中は詠唱攻撃フラグを保ったまま時間を計測しない。
			//再開できるように
		if (useMagic)
		{
		...
		//戦闘中、かつ位置についているなら
		if (sb.nowState == BrainAbility.SisterState.戦い && sb.nowPosition)
		{
				//戦闘開始時にフラグをきれいにしておきます
			if (isReset)
			{
					//これは道中回復とそのサウンドを終わらせてる
	...
	void SecondTargetJudge(List<GameObject> targetList, AttackJudge condition, List<EnemyAIBase> statusList)
	{
		...
		else
		{
			...
			if (condition.nextCondition != AttackJudge.AdditionalJudge.指定なし)
			{
				...
				SManager.instance.target.GetComponent<EnemyAIBase>().TargetEffectCon(2);
			}
			else
			{
	...
	void Combination()
	{
		...
		ca.CombinationStart(conboChain);
...

	{
		...
		protected override void InitializeAnimatorParameters()
		{
			...
			RegisterAnimatorParameter(_castParameterName, AnimatorControllerParameterType.Bool, out _castAnimationParameter);
		}
		...
		public override void UpdateAnimator()
		{
			...
			MMAnimatorExtensions.UpdateAnimatorBool(_animator, _castAnimationParameter, SManager.instance.castNow, _character._animatorParameters);
		}
