...
public class UseController : MonoBehaviour, IEnhancedScrollerDelegate
{
    ...
    [SerializeField] private ToolCellView fooCellViewPrefab;
 ...
 //  int lLimit;
   // int uLimit;
    ...
    {
        var cellView = scroller.GetCellView(this.fooCellViewPrefab) as ToolCellView;
    ...
    private void Start()
    {
        ...
        setList = new List<ToolItem>(ToolList());
        ...
        //Nと同じ数だけ格納するminiListを作成、miniList.size()の最大値 = N
        List<ToolItem> miniList = new List<ToolItem>(N);
    ...
    private void UpdateNavigationConnections()
    {
        ...
        var cells = this.content.Cast<RectTransform>()
            .Select(t => t.GetComponent<ToolCellView>())
