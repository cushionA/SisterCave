...
public class EnemyDataController : MonoBehaviour, IEnhancedScrollerDelegate
{
    ...
    [SerializeField] private EnemyCellView enemyCellViewPrefab;
   ...
   // int jumpDataIndex;
    ...
    {
        var cellView = scroller.GetCellView(this.enemyCellViewPrefab) as EnemyCellView;
    ...
    private void Start()
    {
        ...
        data = null;
        setList = new List<EnemyLibrary>(ToolList());
        ...
        //Nと同じ数だけ格納するminiListを作成、miniList.size()の最大値 = N
        List<EnemyLibrary> miniList = new List<EnemyLibrary>(N);
        ...
        for (int i = 0; i < setList.Count(); i++)
        {
            ...
            {
                miniList = null;
                miniList = new List<EnemyLibrary>(N);
            }
            //miniListに格納
            ...
            if (i % N == N - 1)
            {
                ...
                EnemyData mini = new EnemyData(miniList.ToArray());
                data.Add(mini);
                mini = null;
            }
        }
    ...
    private void UpdateNavigationConnections()
    {
        ...
        var cells = this.content.Cast<RectTransform>()
            .Select(t => t.GetComponent<EnemyCellView>())
    ...
    IEnumerable<EnemyLibrary> ToolList()
    {
