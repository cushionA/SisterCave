...
{
	/// <summary>
	/// このコンポーネントをキャラクターに追加すると、しゃがんだり、這ったりすることができるようになります。
	/// アニメーターパラメーター：しゃがむ、這う
	/// </summary>
	...
	public class PlayerCrouch : MyAbillityBase
	{
		...
		[Tooltip("falseに設定すると、キャラクターは這うことができず、しゃがむだけとなります。")]
		...
		[Tooltip("これが true の場合、コライダーはしゃがんだときにリサイズされます。")]
		...
		[Tooltip("しゃがんだときにコライダに適用するサイズ (ResizeColliderWhenCrouched が true の場合。それ以外の場合は無視されます)"　+
		"しゃがむときにコライダの幅を変更すると、斜面でしゃがみを開始したときにグリッチが発生する可能性が高いので、変更しないほうがよいことに注意してください。")]
		...
		[Tooltip("falseの場合、キャラクターはしゃがみ始めるために移動を停止する必要があります。")]
		...
		[MMReadOnly]
		[Tooltip("もしそうなら、キャラクターはしゃがんでいて、頭の上に障害物があり、再び立ち上がることができません。")]
		...
		[Tooltip("Cinemachine仮想カメラのフォーカスポイントとして使用されるカメラターゲットを移動するかどうか")]
